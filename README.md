...
<img width="2446" height="804" alt="fig2" src="https://github.com/user-attachments/assets/c0d9fa49-f2a0-437f-827d-0b3f3b8a56d8" />
...
(3) Supervised emotion state prediction (regression). After training the LSTM autoencoder, we only used the encoder part to compute the hidden state h_t at each time step t. We then averaged the hidden representations across all time steps within the trial as s_n. We then related the hidden representation s_n  to the valence/arousal rating in that trial, i.e., y_n, via a two-layer multi-layer perceptron (MLP) regressor for fine-grained prediction of continuous valence/arousal ratings.

(4) Supervised emotion state categorization (classification). To classify coarse-grained discretized valence and arousal states, we first transformed the continuous valence or arousal ratings into discrete categories. Similar to the emotion state prediction decoder, we used a two-layer MLP classifier. 
